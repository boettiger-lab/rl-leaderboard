...
Welcome to the Conservation Agents Leaderboard.<br /><br />On this left side-panel, context is provided for the environments from the Conservation Gym. The right side-panel hosts the leaderboard where submitted agents are evaluated.

### conservation-v0

This environment considers a single species conservation problem in which the agent can increase the population at a cost.
...
This environment considers a single species conservation problem that has a non-stationary model. 
...
This environment considers a conservation problem that has a non-stationary model. It is similar to `conservation-v5` but works on an ensemble of populations. By default, the replicate number is 1, so the default is equivalent to `conservation-v5`
